...
type AxisDomain = [number | 'auto' | ((min: number, max: number) => number), number | 'auto' | ((min: number, max: number) => number)];
...
interface ChartProps {
  ...
  pieLabelFormatter?: (params: any) => string;
  disablePieGrouping?: boolean;
}
...
export const BIChart: React.FC<ChartProps> = ({ 
  ...
  const formatValueForTooltip = (value: any, name?: string) => {
    if (!valueFormatter) return value;
    const numericValue = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(numericValue) ? valueFormatter(numericValue, name) : value;
  };
  ...
  const pieData = useMemo(() => {
    if (currentType !== 'pie' || disablePieGrouping) return data;
  ...
  const renderChart = () => {
    ...
    switch (currentType) {
        ...
        return (
          <PieChart>
            <Pie
              ...
              cx="50%"
              cy="50%"
              innerRadius={pieInnerRadius ?? (minimal ? 40 : 60)}
              outerRadius={pieOuterRadius ?? (minimal ? 60 : 90)}
              paddingAngle={5}
              dataKey={dataKey}
              nameKey={categoryKey}
              label={minimal ? false : (props) => {
                if (pieLabelFormatter) return pieLabelFormatter(props);
                const { name, percent } = props;
                return `${name} ${(percent * 100).toFixed(0)}%`;
              }}
              labelLine={!pieLabelFormatter}
            >
              {chartData.map((_, index) => (
                <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
              ))}
            </Pie>
            <Tooltip formatter={(value: any, name: string) => [formatValueForTooltip(value, name), name]} />
            {showLegend && (
              <Legend 
                layout={legendLayout}
                align={legendAlign}
                verticalAlign={legendVerticalAlign}
                formatter={(val: any) => legendFormatter ? legendFormatter(String(val)) : val}
              />
            )}
          </PieChart>
        );
      case 'bar':
        return (
          <ComposedChart data={chartData} layout={layout} margin={chartMargin}>
            ...
            {secondaryDataKey && !mergeAxes && (
              <YAxis yAxisId="right" orientation="right" domain={[0, 100]} fontSize={9} tickFormatter={(val) => `%${val}`} />
            )}
            <Tooltip 
              formatter={(value: any, name: string, props: any) => {
                const payload = props?.payload;
                const formatted = formatValueForTooltip(value, name || title);
                if (payload && typeof payload.pay === 'number') {
                  return [`${formatted} (Pay: %${payload.pay.toFixed(1)})`, 'İşlem Sayısı'];
                }
            ...
            {secondaryDataKey && (
              <Line yAxisId={mergeAxes ? 'left' : 'right'} type="monotone" dataKey={secondaryDataKey} stroke="#f43f5e" strokeWidth={3} dot={{ r: 3 }} />
            )}
          </ComposedChart>
        );
      case 'line':
        return (
          <LineChart data={chartData}>
            ...
            <Tooltip 
              formatter={(value: any, name: string) => [formatValueForTooltip(value, name), name]}
      ...
      <div className="flex flex-col h-full w-full relative">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            {subtitle && <p className="text-[8px] font-black text-slate-300 uppercase tracking-[0.3em] mt-1">{subtitle}</p>}
          </div>
    ...
    <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col h-full chart-container group hover:shadow-lg transition-all duration-300 relative">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          {subtitle && <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em] mt-1">{subtitle}</p>}
        </div>
